...
int px = 7, py = 7; // posición inicial sugerida para el guardián
...
int main()
{
    int tipo_jugador = JUGADOR_GUARDIAN;
    ...
    while ((ch = getch()) != 'q')
    {
        ...
        {
            if (captura_guardian(map[new_py][new_px]))
            {
                mvprintw(HEIGHT + 1, 0, "¡Atrapaste al explorador! Pulsa 'q' para salir.");
            }
            px = new_px;
            py = new_py;
        }
        draw_map();
        mvaddch(py, px, CELDA_GUARDIAN);
        refresh();
    }
    endwin();
